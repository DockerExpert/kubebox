...
function execute(request, { generator, readable, async, cancellable }) {
  const { client, authorization = true, token } = request.context;
  const options = merge({}, client.master_api, request.options);

  if (!authorization/* && !context.jwt */) {
    delete options.headers['Authorization'];
  } else if (token) {
    options.headers['Authorization'] = `Bearer ${token}`;
  }

  if (client.oidc && authorization) {
    // TODO
  ...
  return get(options, { generator, readable, async, cancellable });
}
